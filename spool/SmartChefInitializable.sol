...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    constructor(uint _minStaked2Start) {
        SMART_CHEF_FACTORY = msg.sender;
        minStaked2Start = _minStaked2Start;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(stakedToken.balanceOf(address(this)).add(_amount) <= poolLimit, "Amount above pool limit");
        if (hasUserLimit) {
     ...
     */
    function updatePoolLimitPerUser(bool _hasUserLimit, uint256 _poolLimitPerUser, uint _poolLimit) external onlyOwner {
        if (_hasUserLimit) {
            poolLimitPerUser = _poolLimitPerUser;
        } else {
    ...
    function setMinStaked2Start(uint _minStaked2Start) external onlyOwner {
        minStaked2Start = _minStaked2Start;
    }
    /**
     ...
     */
    function _updatePool(uint _stakeAmount) internal {
        if (block.number <= lastRewardBlock) {
            return;
        }
        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
        if (stakedTokenSupply + _stakeAmount >= minStaked2Start) {
    ...
    function _updatePoolWD() internal {
        if (block.number <= lastRewardBlock) {
            return;
        }
        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
